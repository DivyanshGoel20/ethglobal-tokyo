...
function signedCookie(nullifier: string): string {
  const token = attachSession(NextResponse.json({}), nullifier).cookies.get("float_session")?.value;
  assert.ok(token);
  return `float_session=${token}`;
}
...
test("the session route answers with the human the signed cookie proves", async () => {
  const res = await sessionGET(req("http://float.test/api/auth/session", signedCookie(HUMAN)));
...
test("a bare world_session cookie is not a session", async () => {
  ...
  const res = await sessionGET(req("http://float.test/api/auth/session", `world_session=${HUMAN}`));
...
test("signing out voids the cookie", async () => {
  ...
  assert.match(header, /float_session=;/);
  assert.match(header, /Max-Age=0/i);
});

test("reputation cannot be read by naming a nullifier in the query", async () => {
  const res = await reputationGET(req(`http://float.test/api/reputation?humanOwner=${HUMAN}`));
  assert.equal(res.status, 401);
});

test("reputation is readable by the human it belongs to", async () => {
  const res = await reputationGET(req("http://float.test/api/reputation", signedCookie(HUMAN)));
  assert.equal(res.status, 200);
